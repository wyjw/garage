...
class FirstOrderOptimizer(Serializable):
    ...
    def update_opt(self,
        ...
        with tf.name_scope(
            ...
            if policy:
                self._pol_loss = loss
                self._grads_and_vars = self._tf_optimizer.compute_gradients(
                    loss)
    ...
    def optimize(self, inputs, extra_inputs=None, callback=None, policy=False):
        ...
        for epoch in range(self._max_epochs):
            ...
            for batch in dataset.iterate(update=True):
                ...
                if policy:
                    g = sess.run(
                        tf.gradients(self._pol_loss,
                                     self._target.get_params(trainable=True)),
                        dict(list(zip(self._input_vars, batch))))
                    w = sess.run(self._target.get_params(trainable=True))
                    gv = sess.run([
                        i[0] for i in self._grads_and_vars if i[0] is not None
                    ], dict(list(zip(self._input_vars, batch))))
                    gv = dict(
                        list(
                            zip([
                                i[1].name for i in self._grads_and_vars
                                if i[0] is not None
                            ], gv)))
                    pol_gradients.append(g)
                    weights.append(w)
                    all_grads.append(gv)
            ...
            last_loss = new_loss
        if policy:
            return pol_gradients, weights, all_grads
        return None
